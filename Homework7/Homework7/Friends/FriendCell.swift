...
final class FriendCell: UITableViewCell, FriendCellProtocol {
    ...
    // Вводим для реализации клика по ячейке друга для того, чтобы перейти в его профиль.
    // Для отображения профиля используем тот же контроллер, что и для профиля самого пользователя.
